...
/* ── Configuración ── */
const ADMIN_USER  = "rayami";       // usuario (legacy)
const ADMIN_PASS  = "rayami2025";   // contraseña (legacy)
const ADMIN_EMAIL = "rayami";       // también acepta este como "correo"
...
function initFirebase() {
  ...
  // Sellers — siempre escuchar, renderizar si admin está activo
...
/* ============================================
   HERO SHOWCASE — productos rotativos
============================================================ */
...
function initHeroShowcase() {
  // Tomar hasta 8 productos no vendidos con foto
  heroProducts = allProducts
    .filter(p => !p.sold && (p.photos?.length || p.image))
    .slice(0, 8);
  ...
  showcase.innerHTML = heroProducts.map((p, i) => {
    ...
    const imgHTML = photo
      ...
      : `<div class="hsc-placeholder">${p.emoji || "📦"}</div>`;
    const finalPrice = p.salePrice > 0 && (!p.promoEnd || p.promoEnd > Date.now())
      ? p.salePrice : p.price;
    return `
      <div class="hero-showcase-card ${i === 0 ? "active" : ""}" id="hsc-${i}">
        <div class="hsc-img-wrap">${imgHTML}</div>
        <div class="hsc-info">
          <span class="hsc-tag">${p.category?.toUpperCase()} · ${p.condition}</span>
          <span class="hsc-name">${p.name}</span>
          <span class="hsc-price">${formatPrice(finalPrice)}</span>
        </div>
      </div>`;
  }).join("");

  // Crear dots
  dotsWrap.innerHTML = heroProducts.map((_, i) =>
    `<div class="hsd-dot ${i === 0 ? "active" : ""}" onclick="heroGoTo(${i})"></div>`
  ).join("");

  heroIndex = 0;
  clearInterval(heroInterval);
  heroInterval = setInterval(() => heroGoTo((heroIndex + 1) % heroProducts.length), 3500);
}

window.heroGoTo = function(i) {
  if (!heroProducts.length) return;
  // Quitar active anterior
  document.querySelector(`#hsc-${heroIndex}`)?.classList.remove("active");
  document.querySelectorAll(".hsd-dot")[heroIndex]?.classList.remove("active");
  // Activar nuevo
  heroIndex = i;
  document.querySelector(`#hsc-${heroIndex}`)?.classList.add("active");
  document.querySelectorAll(".hsd-dot")[heroIndex]?.classList.add("active");
  // Reiniciar timer
  clearInterval(heroInterval);
  heroInterval = setInterval(() => heroGoTo((heroIndex + 1) % heroProducts.length), 3500);
};
...
function listenAllSellers(callback) {
  // Solo usado internamente — el listener principal está en initFirebase
  callback(allSellers);
}


/* ============================================
   VISTAS — tracking con localStorage
============================================================ */
const LS_VIEWS_KEY = "rayami_viewed";
...
function markViewed(fbKey) {
  try {
    const viewed = getViewedSet();
    viewed.add(fbKey);
    localStorage.setItem(LS_VIEWS_KEY, JSON.stringify([...viewed]));
  } catch {}
}

async function trackView(fbKey) {
  const viewed = getViewedSet();
  if (viewed.has(fbKey)) return; // ya visto en este dispositivo
  markViewed(fbKey);
  try {
    const viewsRef = ref(db, `views/${fbKey}`);
    const snap = await new Promise(r => { const u = onValue(viewsRef, s => { u(); r(s); }); });
    await set(viewsRef, (snap.val() || 0) + 1);
  } catch(e) {}
}
...
function initViews() {
  onValue(ref(db, "views"), snap => {
    ...
    if (currentSeller) renderSellerTable();
  });
}
...
function renderCard(product) {
  ...
  // Registrar visita (solo clientes, no admin ni vendedor)
  if (product.fbKey && !isLoggedIn && !currentSeller) {
    trackView(product.fbKey);
  }
  ...
  return `
    <div class="product-card" id="card-${carId}">
      ...
      <div class="pc-info">
        ...
        ${product.sellerName ? `<span class="pc-seller-tag">🏪 ${product.sellerName}</span>` : '<span class="pc-seller-tag">🏪 LC</span>'}
...
function initNavbar() {
  window.addEventListener("scroll", () => {
    ...
    if (window.scrollY > 40) {
      nav.style.background = "rgba(10,10,10,.99)";
      nav.classList.add("scrolled");
    } else {
      nav.style.background = "rgba(13,13,13,.96)";
  ...
  document.querySelectorAll(".trust-item, .cat-card, .product-card, .hero-stat").forEach(el => {
...
function showAdmin() {
  ...
  renderAdminStats();
  renderAdminSellers();
  initOrders(); // ya tenemos allSellers cargado

  // Listener en tiempo real para productos de vendedores
  onValue(ref(db, "sellerProducts"), snap => {
    const data = snap.val() || {};
    allSellerProductsList = Object.entries(data).map(([k, v]) => ({ ...v, fbKey: k }));
    // Badge en tab
    const pending = allSellerProductsList.filter(p => !p.approved && !p.rejected);
    const tabEl   = document.getElementById("adminTabSellerProducts");
    if (tabEl) tabEl.textContent = `📋 Por aprobar${pending.length > 0 ? " (" + pending.length + ")" : ""}`;
    // Re-render si el tab está activo
    const panel = document.getElementById("adminPanelSellerProducts");
    if (panel && panel.style.display !== "none") renderSellerProductsTable();
  });
}
...
function renderAdminTable() {
  ...
  tbody.innerHTML = list.map(p => {
    ...
    const vendorName = p.sellerName || "LC";
    const views = allViews[p.fbKey] || 0;
    return `
      <tr class="${p.sold ? "row-sold" : ""}">
        ...
        <td><span class="admin-vendor-tag">🏪 ${vendorName}</span></td>
...
async function saveProduct() {
  ...
  const productData = {
    ...
    promoEnd:      document.getElementById("pPromoEnd").value
                     ? new Date(document.getElementById("pPromoEnd").value).getTime() : 0,
  ...
  try {
    if (editingId) {
      // Edición directa — ya publicado, actualizar en products
      productData.fbKey = editingId;
      await fbSave(productData);
      showToast("✅ Producto actualizado");
    } else {
      // Nuevo — va a sellerProducts pendiente de tu aprobación
      await push(ref(db, "sellerProducts"), {
        ...productData, approved: false, rejected: false
      });
      showToast("✅ Guardado — ve a 'Por aprobar' para publicarlo 🔥");
    }
    closeProductModal();
  } catch(err) {
...
/* ============================================
   CHECKOUT — Flujo de pedido completo
============================================================ */
let checkoutData = {};
...
window.openOrderModal = function() {
  ...
  closeCart();
  ["coName","coPhone","coColonia","coAddress","coNotes"].forEach(id => {
    const el = document.getElementById(id); if(el) el.value = "";
  });
  document.querySelectorAll('input[name="coDelivery"]').forEach(r => r.checked = false);
...
window.selectDelivery = function(type) {
  document.querySelectorAll(".delivery-opt").forEach(o => o.classList.remove("selected"));
  const opt = document.getElementById(type === "domicilio" ? "optDom" : "optPunto");
...
window.goToCheckoutStep2 = function() {
  ...
  const colonia  = document.getElementById("coColonia").value.trim();
  const delivery = document.querySelector('input[name="coDelivery"]:checked')?.value;
  if (!name)     { showToast("⚠️ Escribe tu nombre"); return; }
  if (!phone)    { showToast("⚠️ Escribe tu WhatsApp"); return; }
  if (!colonia)  { showToast("⚠️ Escribe tu colonia"); return; }
  if (!delivery) { showToast("⚠️ Elige cómo quieres recibir tu pedido"); return; }
  checkoutData = {
    name, phone, colonia,
...
function buildOrderStep2() {
  ...
  const deliveryLabel = checkoutData.delivery === "domicilio"
    ? "🏠 Entrega a domicilio (sábado/domingo)"
    : "📍 Punto de encuentro (entre semana)";
  document.getElementById("orderCardDelivery").innerHTML = `
    <div style="margin-top:12px;font-size:12px;color:#888;">
      <div>${deliveryLabel}</div>
      <div>📍 ${checkoutData.colonia}${checkoutData.address?" · "+checkoutData.address:""}</div>
      <div>📱 ${checkoutData.phone}</div>
    </div>`;

  const lines   = cart.map(i=>`• ${i.name} x${i.qty}  →  ${formatPrice(i.price*i.qty)}`).join("\n");
  const delivTxt = checkoutData.delivery==="domicilio"
    ? `🏠 Entrega a domicilio (sábado/domingo)\n📍 ${checkoutData.colonia}${checkoutData.address?"\n🏡 "+checkoutData.address:""}`
    : `📍 Punto de encuentro entre semana\n📌 Colonia: ${checkoutData.colonia}`;

  // Generar código de seguimiento provisional (se reemplaza al guardar)
  const preCode = "RAY-" + Date.now().toString(36).toUpperCase().slice(-6);

  document.getElementById("orderTextarea").value =
`🛍️ *PEDIDO RAYAMI STORE*
📌 Código de seguimiento: *${preCode}*
...
👤 ${checkoutData.name}
📱 ${checkoutData.phone}
${delivTxt}${checkoutData.notes?"\n📝 "+checkoutData.notes:""}

💳 DATOS DE PAGO:
Banco: *Banorte*
Cuenta: *072180012986149248*
Titular: *RAYAMI STORE*
Monto a depositar: *${formatPrice(sum)}*

⚠️ Adjunta tu comprobante de pago a este mensaje para confirmar tu pedido.
Puedes rastrear tu pedido en: rayami-07.github.io/store/#seguimiento-${preCode}`;
}

window.sendOrderWA = async function() {
  const sum = cart.reduce((s,i) => s+i.price*i.qty, 0);

  // Generar código de seguimiento único
  const trackCode = "RAY-" + Date.now().toString(36).toUpperCase().slice(-6) +
                    Math.random().toString(36).toUpperCase().slice(2,4);

  const orderRef = await push(ref(db,"orders"), {
    ...checkoutData,
    items:      cart.map(i=>({name:i.name,price:i.price,qty:i.qty,fbKey:i.fbKey})),
    total:      sum,
    status:     "pendiente",
    trackCode,
    createdAt:  Date.now(),
  ...
  // Actualizar mensaje con código real
  const lines   = cart.map(i=>`• ${i.name} x${i.qty}  →  ${formatPrice(i.price*i.qty)}`).join("\n");
  const delivTxt = checkoutData.delivery==="domicilio"
    ? `🏠 Entrega a domicilio (sábado/domingo)\n📍 ${checkoutData.colonia}${checkoutData.address?"\n🏡 "+checkoutData.address:""}`
    : `📍 Punto de encuentro entre semana\n📌 Colonia: ${checkoutData.colonia}`;
...
⚠️ Adjunta tu comprobante de pago a este mensaje.
Rastrea tu pedido en: rayami-07.github.io/store/#seguimiento-${trackCode}`;

  window.open(`https://api.whatsapp.com/send?phone=${WA_NUMBER}&text=${encodeURIComponent(text)}`,"_blank");

  cart = [];
  allProducts.forEach(p => updateCartBtn(p.fbKey));
  renderCartBadge();
  closeOrderModal();
  showToast(`✅ Pedido enviado · Código: ${trackCode} 🔥`, 5000);
};

window.copyAccount = function() {
  navigator.clipboard.writeText("072180012986149248").then(() => {
...
window.saveOrderImage = function() {
  ...
  html2canvas(card,{backgroundColor:"#1a1a1a",scale:2,useCORS:true,logging:false})
    .then(canvas => {
    ...
    }).catch(() => showToast("❌ Error. Usa 'Copiar texto'"));
};
...
function initOrders() {
  onValue(ref(db,"orders"), snap => {
    const data = snap.val() || {};
    allOrders = Object.entries(data)
      .map(([k,v]) => ({...v,fbKey:k}))
      .sort((a,b) => (b.createdAt||0)-(a.createdAt||0));
    const panel = document.getElementById("adminPanelOrders");
    if (panel && panel.style.display !== "none") renderOrdersTable();
    const pending = allOrders.filter(o=>o.status==="pendiente").length;
    const tabEl   = document.getElementById("adminTabOrders");
    if (tabEl) tabEl.textContent = `🛵 Pedidos${pending>0?" ("+pending+")":""}`;
  });
}
...
window.switchOrdersView = function(view) {
  document.getElementById("ordersViewTabla").style.display = view==="tabla" ? "block" : "none";
  document.getElementById("ordersViewRuta").style.display  = view==="ruta"  ? "block" : "none";
  if (view==="ruta") renderRouteView();
};
...
function renderOrdersTable() {
  ...
  document.getElementById("oStatTotal").textContent     = allOrders.filter(o=>o.status!=="cancelado").length;
  document.getElementById("oStatPendiente").textContent = allOrders.filter(o=>o.status==="pendiente").length;
  document.getElementById("oStatPagado").textContent    = allOrders.filter(o=>o.status==="pagado").length;
  document.getElementById("oStatEntregado").textContent = allOrders.filter(o=>o.status==="entregado").length;
  ...
  const statusMap = {
    pendiente: `<span class="pub-badge pub-wait">⏳ Pendiente</span>`,
    pagado:    `<span class="pub-badge pub-ok">✅ Pagado</span>`,
    en_camino: `<span class="pub-badge" style="background:rgba(255,123,28,.15);color:var(--accent);">🛵 En camino</span>`,
    entregado: `<span class="pub-badge pub-ok">📦 Entregado</span>`,
    cancelado: `<span class="pub-badge pub-no">❌ Cancelado</span>`,
  };

  tbody.innerHTML = filtered.map((o) => {
    const items    = (o.items||[]).map(i=>`${i.name} x${i.qty}`).join(", ");
    const delivery = o.delivery==="domicilio" ? "🏠 Domicilio" : "📍 Punto medio";
    const next     = {pendiente:"pagado", pagado:"en_camino", en_camino:"entregado"}[o.status];
    const nextLbl  = {pagado:"✅ Pagado", en_camino:"🛵 En camino", entregado:"📦 Entregado"}[next];
    const num      = allOrders.length - allOrders.indexOf(o);
    const cancelBtn = o.status !== "entregado" && o.status !== "cancelado"
      ? `<button class="btn-delete" style="background:rgba(255,68,68,.1);color:var(--red);border-color:rgba(255,68,68,.2);" onclick="cancelOrder('${o.fbKey}')">❌ Cancelar</button>`
      : "";

    return `<tr class="${o.status==="cancelado"?"row-sold":""}">
      <td style="font-size:12px;color:var(--text-muted);">#${num}</td>
      <td>
        <strong>${o.name}</strong><br/>
        <a href="https://api.whatsapp.com/send?phone=52${(o.phone||"").replace(/\D/g,"")}" target="_blank" style="color:var(--wa);font-size:12px;">📱 ${o.phone}</a>
      </td>
      <td style="font-size:12px;max-width:160px;">${items}</td>
      <td><span class="admin-price">${formatPrice(o.total||0)}</span></td>
      <td style="font-size:12px;">${delivery}<br/><span style="color:var(--text-muted)">${o.colonia||""}</span></td>
      <td style="font-size:12px;color:var(--text-muted);">${o.fecha||""}</td>
      <td>${statusMap[o.status]||o.status}</td>
      <td><div class="admin-actions">
        ${next ? `<button class="btn-edit" onclick="updateOrderStatus('${o.fbKey}','${next}')">${nextLbl}</button>` : ""}
        ${cancelBtn}
        <button class="btn-delete" onclick="deleteOrder('${o.fbKey}')">🗑</button>
      </div></td>
    </tr>`;
  }).join("");
}

window.cancelOrder = async function(fbKey) {
  const order = allOrders.find(o=>o.fbKey===fbKey);
  if (!order) return;
  if (!confirm(`¿Cancelar el pedido de ${order.name}?`)) return;
  const {fbKey:_k,...data} = order;
  await set(ref(db,`orders/${fbKey}`), {...data, status:"cancelado"});
  // Notificar al cliente
  if (order.phone) {
    const msg = `Hola ${order.name}, tu pedido en RAYAMI STORE fue cancelado. Si tienes dudas escríbenos. 🙏`;
    window.open(`https://api.whatsapp.com/send?phone=52${order.phone.replace(/\D/g,"")}&&text=${encodeURIComponent(msg)}`,"_blank");
  }
...
function renderRouteView() {
  // Solo pedidos pagados y en camino (activos)
  const active = allOrders.filter(o => o.status === "pagado" || o.status === "en_camino");

  // PASO 1 — Recolección: agrupar por vendedor
  const vendorMap = {};
  active.forEach(order => {
    (order.items||[]).forEach(item => {
      // Buscar vendedor del producto
      const product = allProducts.find(p => p.fbKey === item.fbKey);
      const sellerName = product?.sellerName || "LC";
      const sellerKey  = product?.sellerKey  || "admin";
      if (!vendorMap[sellerKey]) {
        vendorMap[sellerKey] = { name: sellerName, items: [], phone: "" };
        // Buscar teléfono del vendedor
        const seller = allSellers.find(s => s.fbKey === sellerKey);
        if (seller) vendorMap[sellerKey].phone = seller.phone || "";
      }
      vendorMap[sellerKey].items.push({ ...item, orderName: order.name });
    });
  });

  const recoleccionBody = document.getElementById("rutaRecoleccionBody");
  const recoleccionEmpty = document.getElementById("rutaRecoleccionEmpty");
  const vendors = Object.entries(vendorMap);

  if (!vendors.length) {
    recoleccionBody.innerHTML = "";
    recoleccionEmpty.style.display = "block";
  } else {
    recoleccionEmpty.style.display = "none";
    recoleccionBody.innerHTML = vendors.map(([key, v], i) => {
      const productos = v.items.map(it => `${it.name} x${it.qty} (para ${it.orderName})`).join("<br/>");
      const waLink = v.phone
        ? `<a href="https://api.whatsapp.com/send?phone=52${v.phone.replace(/\D/g,"")}&text=${encodeURIComponent(`Hola ${v.name}! Paso a recoger el pedido pronto 🛵`)}" target="_blank" style="color:var(--wa)">📱 Avisar</a>`
      ...
      return `<tr>
        ...
        <td>${waLink}</td>
        <td><input type="checkbox" style="width:18px;height:18px;accent-color:var(--accent);" /></td>
      </tr>`;
    }).join("");
  }

  // PASO 2 — Entregas a domicilio (ordenadas por colonia)
  const domicilio = active
    .filter(o => o.delivery === "domicilio")
    .sort((a,b) => (a.colonia||"").localeCompare(b.colonia||""));

  const domBody  = document.getElementById("rutaDomicilioBody");
  const domEmpty = document.getElementById("rutaDomicilioEmpty");

  if (!domicilio.length) {
    domBody.innerHTML = ""; domEmpty.style.display = "block";
  } else {
    domEmpty.style.display = "none";
    domBody.innerHTML = domicilio.map((o,i) => {
      const items = (o.items||[]).map(it=>`${it.name} x${it.qty}`).join(", ");
      const waLink = `<a href="https://api.whatsapp.com/send?phone=52${(o.phone||"").replace(/\D/g,"")}&text=${encodeURIComponent(`Hola ${o.name}! Ya voy en camino con tu pedido 🛵`)}" target="_blank" style="color:var(--wa)">📱 Avisar</a>`;
      return `<tr>
        <td><strong>${i+1}</strong></td>
        <td><strong>${o.name}</strong></td>
        <td><span style="color:var(--accent);font-weight:600;">${o.colonia||"—"}</span></td>
  ...
  // PASO 3 — Puntos de encuentro (ordenados por colonia)
  const puntos = active
    .filter(o => o.delivery === "punto")
    .sort((a,b) => (a.colonia||"").localeCompare(b.colonia||""));

  const puntoBody  = document.getElementById("rutaPuntoBody");
  const puntoEmpty = document.getElementById("rutaPuntoEmpty");

  if (!puntos.length) {
    puntoBody.innerHTML = ""; puntoEmpty.style.display = "block";
  } else {
    puntoEmpty.style.display = "none";
    puntoBody.innerHTML = puntos.map((o,i) => {
      const items = (o.items||[]).map(it=>`${it.name} x${it.qty}`).join(", ");
      const waLink = `<a href="https://api.whatsapp.com/send?phone=52${(o.phone||"").replace(/\D/g,"")}&text=${encodeURIComponent(`Hola ${o.name}! Coordinemos el punto de encuentro 📍`)}" target="_blank" style="color:var(--wa)">📱 Coordinar</a>`;
      return `<tr>
        <td><strong>${i+1}</strong></td>
        <td><strong>${o.name}</strong></td>
        <td><span style="color:var(--accent);font-weight:600;">${o.colonia||"—"}</span></td>
  ...
  // Resumen del día
  const totalDia = active.reduce((s,o) => s+(o.total||0), 0);
  document.getElementById("rutaResumen").innerHTML = `
    <div class="ruta-resumen-inner">
      <div class="ruta-resumen-item"><span>📦 Recolecciones</span><strong>${vendors.length} vendedores</strong></div>
      <div class="ruta-resumen-item"><span>🏠 A domicilio</span><strong>${domicilio.length} pedidos</strong></div>
      <div class="ruta-resumen-item"><span>📍 Punto de encuentro</span><strong>${puntos.length} pedidos</strong></div>
      <div class="ruta-resumen-item"><span>💰 Total a cobrar</span><strong style="color:var(--accent);font-size:22px;">${formatPrice(totalDia)}</strong></div>
    </div>`;
}

window.updateOrderStatus = async function(fbKey, newStatus) {
  const order = allOrders.find(o=>o.fbKey===fbKey);
  if (!order) return;
  const {fbKey:_k,...data} = order;
  await set(ref(db,`orders/${fbKey}`), {...data, status:newStatus});
  if (newStatus==="en_camino" && order.phone) {
    const msg = `Hola ${order.name}! 👋 Tu pedido de RAYAMI STORE ya va en camino 🛵\n¡Prepárate para recibirlo!`;
    window.open(`https://api.whatsapp.com/send?phone=52${order.phone.replace(/\D/g,"")}&&text=${encodeURIComponent(msg)}`,"_blank");
  }
  if (newStatus==="entregado" && order.phone) {
    const msg = `Hola ${order.name}! ✅ Tu pedido fue entregado. ¡Gracias por comprar en RAYAMI STORE! 🔥`;
    window.open(`https://api.whatsapp.com/send?phone=52${order.phone.replace(/\D/g,"")}&&text=${encodeURIComponent(msg)}`,"_blank");
  }
  showToast("✅ Estado actualizado");
};

window.deleteOrder = async function(fbKey) {
...
function checkDeepLink() {
  ...
  // Seguimiento
  if (hash.startsWith("#seguimiento-")) {
    const code = hash.replace("#seguimiento-","");
    showTrackingView(code);
    return;
  }
...
async function showTrackingView(code) {
  document.getElementById("clientView").style.display  = "none";
  document.getElementById("trackingView").style.display = "block";

  // Aplicar logo
  const logoEl = document.getElementById("trackNavLogo");
  if (logoEl && LOGO_URL) logoEl.src = LOGO_URL;

  // Buscar pedido por código
  const snap = await new Promise(r => { const u = onValue(ref(db,"orders"), s => { u(); r(s); }); });
  const data  = snap.val() || {};
  const entry = Object.entries(data).find(([,v]) => v.trackCode === code);

  const iconEl  = document.getElementById("trackingIcon");
  ...
  if (!entry) {
    iconEl.textContent  = "❓";
    titleEl.textContent = "Pedido no encontrado";
    codeEl.innerHTML    = `<span style="color:var(--text-muted)">Código: ${code}</span>`;
    stepsEl.innerHTML   = `<p style="color:var(--text-muted);text-align:center;font-size:14px;">Verifica que el código sea correcto o contáctanos por WhatsApp.</p>`;
    return;
  }

  const order = { ...entry[1], fbKey: entry[0] };

  const statusConfig = {
    pendiente: { icon:"⏳", title:"Pendiente de pago",    color:"var(--text-muted)", desc:"Tu pedido fue recibido. Estamos esperando tu comprobante de pago." },
    pagado:    { icon:"✅", title:"Pago confirmado",       color:"#25D366",           desc:"¡Tu pago fue confirmado! Tu pedido está siendo preparado." },
    en_camino: { icon:"🛵", title:"En camino",             color:"var(--accent)",     desc:"¡Tu pedido ya va en camino! Pronto lo recibirás." },
    entregado: { icon:"📦", title:"Entregado",             color:"#25D366",           desc:"¡Tu pedido fue entregado! Gracias por comprar en RAYAMI STORE 🔥" },
    cancelado: { icon:"❌", title:"Pedido cancelado",      color:"var(--red)",        desc:"Este pedido fue cancelado. Contáctanos si tienes dudas." },
  };
  ...
  codeEl.innerHTML    = `<span class="tracking-code-badge">📌 ${code}</span>`;

  // Pasos del seguimiento
  const steps = ["pendiente","pagado","en_camino","entregado"];
  const currentIdx = steps.indexOf(order.status);
  stepsEl.innerHTML = `
    <div class="tracking-steps-list">
      ${steps.map((s,i) => {
        const labels = { pendiente:"Pedido recibido", pagado:"Pago confirmado", en_camino:"En camino", entregado:"Entregado" };
        const done   = i <= currentIdx && order.status !== "cancelado";
        const active = i === currentIdx && order.status !== "cancelado";
        return `
          <div class="tracking-step ${done?"done":""} ${active?"active":""}">
            <div class="tracking-step-dot">${done ? "✓" : i+1}</div>
            <span>${labels[s]}</span>
          </div>`;
      }).join('<div class="tracking-step-line"></div>')}
    </div>
    <p style="color:var(--text-muted);font-size:13px;text-align:center;margin-top:16px;">${cfg.desc}</p>`;

  // Info del pedido
  const items = (order.items||[]).map(i=>`<li>${i.name} x${i.qty} — ${formatPrice(i.price*i.qty)}</li>`).join("");
  const delivery = order.delivery==="domicilio" ? "🏠 Entrega a domicilio" : "📍 Punto de encuentro";
  infoEl.innerHTML = `
    <div class="tracking-info-box">
      <div class="tracking-info-row"><span>👤 Cliente</span><strong>${order.name}</strong></div>
      <div class="tracking-info-row"><span>📅 Fecha</span><strong>${order.fecha||"—"}</strong></div>
      <div class="tracking-info-row"><span>🚚 Entrega</span><strong>${delivery}</strong></div>
      <div class="tracking-info-row"><span>📍 Zona</span><strong>${order.colonia||"—"}</strong></div>
      <div class="tracking-info-row"><span>💰 Total</span><strong style="color:var(--accent)">${formatPrice(order.total||0)}</strong></div>
      <ul style="margin-top:12px;padding-left:20px;font-size:13px;color:var(--text-muted);">${items}</ul>
    </div>`;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Drag & drop vendedor
  const spDrop = document.querySelector("#spPhotoTabFile .file-drop");
  if (spDrop) {
    spDrop.addEventListener("dragover", (e) => { e.preventDefault(); spDrop.style.borderColor="var(--accent)"; });
    ...
    spDrop.addEventListener("drop", (e) => {
      ...
      const files = Array.from(e.dataTransfer.files).filter(f => f.type.startsWith("image/"));
      if (files.length) processSellerFiles(files);
      else showToast("⚠️ Solo imágenes");
    });
  }
});
...
/* ============================================
   MODAL PRODUCTO VENDEDOR — fotos
============================================================ */
let sellerCurrentPhotos = [];

window.switchSellerPhotoTab = function(tab) {
  document.getElementById("spPhotoTabUrl").style.display  = tab === "url"  ? "block" : "none";
  document.getElementById("spPhotoTabFile").style.display = tab === "file" ? "block" : "none";
  document.getElementById("spTabBtnUrl").classList.toggle("active",  tab === "url");
  document.getElementById("spTabBtnFile").classList.toggle("active", tab === "file");
};

window.addSellerPhotoFromUrl = function() {
  const url = document.getElementById("spImageUrl").value.trim();
  if (!url) { showToast("⚠️ Pega una URL primero"); return; }
  if (sellerCurrentPhotos.length >= 5) { showToast("⚠️ Máximo 5 fotos"); return; }
  if (sellerCurrentPhotos.includes(url)) { showToast("⚠️ Ya está agregada"); return; }
  sellerCurrentPhotos.push(url);
  document.getElementById("spImageUrl").value = "";
  renderSellerPhotosGallery();
  showToast("✅ Foto agregada");
};

window.handleSellerFileSelect = function(event) {
  const files = Array.from(event.target.files).filter(f => f.type.startsWith("image/"));
  processSellerFiles(files);
};

async function processSellerFiles(files) {
  const remaining = 5 - sellerCurrentPhotos.length;
  if (remaining <= 0) { showToast("⚠️ Ya tienes 5 fotos"); return; }
  const toProcess = files.slice(0, remaining);
  const dropText = document.getElementById("spFileDropText");
  dropText.textContent = `⏳ Subiendo ${toProcess.length} foto${toProcess.length > 1 ? "s" : ""}...`;
  showToast(`⏳ Subiendo ${toProcess.length} foto${toProcess.length > 1 ? "s" : ""}...`);
  ...
  renderSellerPhotosGallery();
  dropText.textContent = "📁 Toca aquí o arrastra más imágenes";
  showToast(`✅ Fotos subidas 🔥`);
}

function renderSellerPhotosGallery() {
  const gallery = document.getElementById("spPhotosGallery");
  const hint    = document.getElementById("spPhotoCountHint");
  if (!sellerCurrentPhotos.length) { gallery.innerHTML=""; hint.style.display="none"; return; }
  hint.style.display  = "block";
  hint.textContent    = `${sellerCurrentPhotos.length}/5 fotos · La primera es la principal`;
  gallery.innerHTML   = sellerCurrentPhotos.map((src, i) => `
    <div class="photo-thumb-wrap">
      <img src="${src}" alt="foto ${i+1}" />
      ${i === 0 ? `<span class="photo-main-badge">Principal</span>` : ""}
      <span class="photo-thumb-remove" onclick="removeSellerPhotoAt(${i})">✕</span>
    </div>`).join("");
}

window.removeSellerPhotoAt = function(i) {
  sellerCurrentPhotos.splice(i, 1);
  renderSellerPhotosGallery();
};
...
window.saveSellerProduct = async function(asDraft = false) {
  ...
  const data = {
    ...
    photos:     sellerCurrentPhotos,
    image:      sellerCurrentPhotos[0] || "",
...
window.saveSellerDraft     = function() { window.saveSellerProduct(true); };
window.submitSellerProduct = function() { window.saveSellerProduct(false); };
...
window.switchAdminTab = function(tab) {
  const panels = {
    ...
    sellers:        "adminPanelSellers",
    sellerProducts: "adminPanelSellerProducts"
  };
  const tabs = {
    products:       "adminTabProducts",
    orders:         "adminTabOrders",
    sellers:        "adminTabSellers",
    sellerProducts: "adminTabSellerProducts"
  };
  ...
  if (tab === "sellerProducts") renderSellerProductsTable();
...
function renderAdminSellers() {
  ...
  document.getElementById("adminSellersList").style.display  = "block";
  document.getElementById("adminSellerDetail").style.display = "none";

  const lcProducts = allProducts.filter(p => p.sellerKey === "admin");
  const lcViews    = lcProducts.reduce((s,p) => s + (allViews[p.fbKey]||0), 0);
  const lcRow = `
    <tr style="background:rgba(255,123,28,.04);cursor:pointer;" onclick="showSellerDetail('admin','LC','Tu cuenta admin')">
      <td><strong>LC</strong> <span style="font-size:11px;color:var(--accent)">👑 Admin</span></td>
      <td>—</td><td>—</td>
      <td><strong>${lcProducts.length}</strong></td>
      <td><span class="views-count">👁 ${lcViews}</span></td>
      <td><span class="pub-badge pub-ok">✅ Activo</span></td>
      <td>—</td>
    </tr>`;

  if (!allSellers.length) { tbody.innerHTML=lcRow; empty.style.display="none"; return; }
  empty.style.display = "none";

  const rows = allSellers.map(s => {
    const phone      = s.phone ? s.phone.replace(/\D/g,'') : "";
    const products   = allProducts.filter(p => p.sellerKey === s.fbKey);
    const totalViews = products.reduce((sum,p) => sum+(allViews[p.fbKey]||0), 0);
    ...
    const approveBtn = s.status!=="active"
      ? `<button class="btn-edit" onclick="event.stopPropagation();approveSeller('${s.fbKey}')">✅ Aprobar</button>` : "";
    const rejectBtn  = `<button class="btn-delete" onclick="event.stopPropagation();rejectSeller('${s.fbKey}')">🚫</button>`;
    return `
      <tr style="cursor:pointer;" onclick="showSellerDetail('${s.fbKey}','${s.name.replace(/'/g,"\\'")}','${(s.email||"").replace(/'/g,"\\'")} · ${s.phone||""}')">
        <td><strong>${s.name}</strong></td>
        <td>${phone?`<a href="https://wa.me/52${phone}" target="_blank" style="color:var(--wa)" onclick="event.stopPropagation()">📱 ${s.phone}</a>`:"—"}</td>
        <td style="font-size:12px;color:var(--text-muted)">${s.email||"—"}</td>
        <td><strong>${products.length}</strong></td>
        <td><span class="views-count">👁 ${totalViews}</span></td>
        <td>${badge}</td>
        <td><div class="admin-actions">${approveBtn}${rejectBtn}</div></td>
      </tr>`;
  }).join("");

  tbody.innerHTML = lcRow + rows;
}

window.showSellerDetail = function(sellerKey, name, info) {
  document.getElementById("adminSellersList").style.display  = "none";
  document.getElementById("adminSellerDetail").style.display = "block";
  document.getElementById("sellerDetailName").textContent    = `🏪 ${name}`;
  document.getElementById("sellerDetailInfo").textContent    = info;

  const products = allProducts.filter(p => p.sellerKey === sellerKey);
  const pending  = allSellerProductsList.filter(p => p.sellerKey === sellerKey && !p.approved);
  const tbody    = document.getElementById("sellerDetailBody");
  const empty    = document.getElementById("sellerDetailEmpty");
  const all      = [
    ...products.map(p => ({...p, _type:"published"})),
    ...pending.map(p  => ({...p, _type: p.draft?"draft":"pending"}))
  ];
  ...
  tbody.innerHTML = all.map(p => {
    ...
    const thumb   = photos.length
      ? `<img class="admin-thumb" src="${photos[0]}" alt="${p.name}">`
      : `<div class="admin-thumb-placeholder">📦</div>`;
    const views   = p._type==="published" ? (allViews[p.fbKey]||0) : 0;
    const status  = p._type==="published"
      ? `<span class="pub-badge pub-ok">✅ En tienda</span>`
      : p._type==="draft"
      ? `<span class="pub-badge" style="background:rgba(107,133,181,.15);color:#6b85b5;">📝 Borrador</span>`
      : `<span class="pub-badge pub-wait">⏳ En revisión</span>`;
    const actions = p._type!=="published"
      ? `<button class="btn-edit" onclick="approveSellerProduct('${p.fbKey}')">✅ Aprobar</button>
         <button class="btn-delete" onclick="rejectSellerProduct('${p.fbKey}')">❌</button>`
      : `<button class="btn-delete" onclick="openDeleteModal('${p.fbKey}')">🗑</button>`;
    return `<tr>
      <td>${thumb}</td>
...
window.backToSellersList = function() {
  document.getElementById("adminSellersList").style.display  = "block";
  document.getElementById("adminSellerDetail").style.display = "none";
};
...
window.approveSeller = async function(fbKey) {
  ...
  if (!seller) return;
  const { fbKey: _k, ...data } = seller;
  await set(ref(db, `sellers/${fbKey}`), { ...data, status: "active" });
...
window.rejectSeller = async function(fbKey) {
  ...
  if (!seller) return;
  const { fbKey: _k, ...data } = seller;
  await set(ref(db, `sellers/${fbKey}`), { ...data, status: "rejected" });
...
function renderSellerProductsTable() {
  ...
  const products = allSellerProductsList;
...
document.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    console.error("Error al iniciar RAYAMI:", err);
  }
});

/* Bind all checkout/order events (needed because script is type=module) */
function bindCheckoutEvents() {
  // Delivery radio options
  document.querySelectorAll('input[name="coDelivery"]').forEach(radio => {
    radio.addEventListener("change", () => window.selectDelivery(radio.value));
  });

  // Checkout step buttons
  const btnStep2 = document.querySelector("#checkoutStep1 .btn-primary");
  if (btnStep2) btnStep2.addEventListener("click", () => window.goToCheckoutStep2());

  const btnBack = document.querySelector("#checkoutStep2 .btn-cancel");
  if (btnBack) btnBack.addEventListener("click", () => window.backToStep1());

  const btnSendWA = document.querySelector("#checkoutStep2 .cart-wa-btn");
  if (btnSendWA) btnSendWA.addEventListener("click", () => window.sendOrderWA());

  const btnCopy = document.getElementById("btnCopy");
  if (btnCopy) btnCopy.addEventListener("click", () => window.copyOrderText());

  const btnImg = document.querySelector(".btn-save-img");
  if (btnImg) btnImg.addEventListener("click", () => window.saveOrderImage());

  // Order modal close
  const closeOrder = document.querySelector("#orderModal .modal-close");
  if (closeOrder) closeOrder.addEventListener("click", () => window.closeOrderModal());

  // Seller login modal
  const showSellerBtn = document.querySelectorAll(".btn-seller-nav, .mm-seller");
  showSellerBtn.forEach(b => b.addEventListener("click", () => window.showSellerLogin()));

  // Admin login
  const showAdminBtns = document.querySelectorAll(".btn-admin-nav, .mm-admin");
  showAdminBtns.forEach(b => b.addEventListener("click", () => window.showLogin()));
}
