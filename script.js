...
function renderAdminSellers() {
  ...
  // LC siempre aparece primero como fila fija
  const lcProducts = allProducts.filter(p => p.sellerKey === "admin").length;
  const lcRow = `
    <tr style="background:rgba(255,123,28,.04);">
      <td><strong>LC</strong> <span style="font-size:11px;color:var(--accent)">👑 Admin</span></td>
      <td><span style="color:var(--text-muted)">Tu cuenta</span></td>
      <td style="font-size:12px;color:var(--text-muted)">—</td>
      <td>${lcProducts}</td>
      <td><span class="pub-badge pub-ok">✅ Activo</span></td>
      <td><span style="font-size:12px;color:var(--text-muted)">Admin principal</span></td>
    </tr>`;

  if (!allSellers.length) {
    tbody.innerHTML = lcRow;
    empty.style.display = "none";
    return;
  }
  empty.style.display = "none";

  const sellersRows = allSellers.map(s => {
    ...
    const productCount = allProducts.filter(p => p.sellerKey === s.fbKey).length;
    return `
      <tr>
        <td><strong>${s.name}</strong></td>
        <td>${phone ? `<a href="https://wa.me/52${phone}" target="_blank" style="color:var(--wa)">📱 ${s.phone}</a>` : "—"}</td>
  ...
  tbody.innerHTML = lcRow + sellersRows;
}
